        ...
        {visibleProjects.map((project) => {
          ...
          return (
            <Link
              ...
              className={`group w-[min(84vw,332px)] min-w-[300px] max-w-[340px] flex-shrink-0 snap-start rounded-[24px] border p-4 transition duration-300 hover:-translate-y-1 hover:shadow-[0_30px_64px_rgba(31,68,116,0.22)] md:w-[304px] md:min-w-[304px] ${style.card}`}
            >
              ...
              <div className={`mt-4 rounded-[18px] border px-3.5 py-3 ${style.panel}`}>
                <p className={`whitespace-nowrap text-[10px] font-semibold uppercase tracking-[0.16em] ${isDark ? "text-[#C9D4F5]" : "text-[#667085]"}`}>
                  Safe to use
                </p>
                <p className={`mt-1 whitespace-nowrap text-[24px] font-semibold tracking-[-0.055em] ${isDark ? "text-[#F7FFC8]" : "text-[#121417]"}`}>{project.remaining}</p>
              </div>

              <div className="mt-4 space-y-2">
                <p className={`flex min-w-0 items-center gap-2 text-[12px] leading-[1.45] ${isDark ? "text-[#DCE8FF]" : "text-[#526173]"}`}>
              ...
              <div className={`mt-4 inline-flex max-w-full items-center gap-2 rounded-full px-3 py-2 text-[12px] font-semibold transition group-hover:translate-x-0.5 ${style.action}`}>
