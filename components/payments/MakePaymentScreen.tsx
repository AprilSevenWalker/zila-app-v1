    ...
    <div className="-mx-4 -mt-2 flex min-h-[calc(100vh-7.5rem)] flex-col overflow-hidden bg-[linear-gradient(160deg,#0D142B_0%,#171E46_42%,#1F2559_72%,#15374F_100%)] px-6 pb-28 pt-8 text-white">
      ...
      <div className="relative flex flex-1 flex-col">
        ...
        <h1 className="mt-4 max-w-[320px] text-[40px] font-semibold leading-[0.98] tracking-[-0.055em] text-white">
          {connected ? "Send a payment from your available cash" : "Connect an account before making a payment"}
        </h1>
        <p className="mt-5 max-w-[320px] text-[16px] leading-[1.68] text-[#D4DCEF]/84">
          {connected
            ? "Mock flow for now. Zila will help route the payment and confirm the timing before any real integration is added."
            : "Set up a money source first so payments and verified activity can be recorded cleanly."}
        </p>

        {!connected ? (
          <div className="mt-8 max-w-[520px]">
            <MoneySourceSetupCard variant="dark" compact />
          </div>
        ) : (
        ...
        <div className="mt-auto flex gap-3 pt-10">
          ...
          <Link
            href={connected ? "/move-funds" : "/wallet"}
          ...
          >
            {connected ? "Continue" : "Connect account"}
          </Link>
        </div>
      </div>
    </div>
